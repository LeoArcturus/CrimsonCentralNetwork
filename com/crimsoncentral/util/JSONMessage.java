...
public class JSONMessage {
	 ...
	 *            The message to send
	 * @param players
	 *            The players you want to send it to
	 * 
	 */
	 ...
	 *
	 * @param players
	 *            The players you want to send this to
	 */
	public void send(Player... players) {
		ReflectionHelper.sendPacket(ReflectionHelper.Packet(toString()), players);
	}
	 ...
	 *
	 * @param fadeIn
	 *            How many ticks to fade in
	 * @param stay
	 *            How many ticks to stay
	 * @param fadeOut
	 *            How many ticks to fade out
	 * @param players
	 *            The players to send this to
	 */
	...
	/**
	 * Makes the action item run a command.
	 *
	 * @param command
	 *            The action to run
	 * @return This {@link JSONMessage} instance
	 */
	public JSONMessage runActionItem(ActionItem a) {
		runCommand("/" + new TemporaryCommand("", a).getName());
		return this;
	}
	 ...
	 *
	 * @param text
	 *            The text to start the next {@link MessagePart} with
	 * @return This {@link JSONMessage} instance
	 */
	...
	public JSONMessage center() {
		String message = "";

		for (MessagePart p : parts) {

			message = message + p.getText();
		}
		...
		int toCompensate = ArenaUtil.CENTER_PX - halvedMessageSize;
		...
		JSONMessage jm = JSONMessage.create(sb.toString());
	...
	private static class ReflectionHelper {
		...
		static Object createActionbarPacket(String message) {
			...
			Object packet = Packet(message);
			setType(packet, (byte) 2);
			return packet;
		}

		static Object Packet(String message) {
			if (!SETUP) {
				throw new IllegalStateException("ReflectionHelper is not set up!");
			}
			try {
		 ...
		 *
		 * @param message
		 *            The text to convert to a chat component
		 ...
		 * @param obj
		 *            The object to change the field of
		 * @param value
	...
	public class MessagePart {
		...
		public void setColor(ChatColor color) {
			if (!color.isColor() && color != ChatColor.BOLD) {
	...
	private static class TemporaryCommand extends BukkitCommand {
		...
		public TemporaryCommand(String name, ActionItem ai) {
			super(name);
			this.description = "Perform Action Item: " + ai;
			this.usageMessage = "/" + name + ai + ArenaUtil.decideArenaName().replace("Arena-", "")
					+ ArenaUtil.decideArenaName().replace("Arena-", "")
					+ ArenaUtil.decideArenaName().replace("Arena-", "")
					+ ArenaUtil.decideArenaName().replace("Arena-", "")
					+ ArenaUtil.decideArenaName().replace("Arena-", "")
					+ ArenaUtil.decideArenaName().replace("Arena-", "");
			...
			this.ai = ai;
			this.cmd = usageMessage.replace("/", "");
			this.setName(cmd);

			try {
				final Field bukkitCommandMap = Main.plugin.getServer().getClass().getDeclaredField("commandMap");
